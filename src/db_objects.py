...
class Location(object):
    def __init__(self, city="", region_code="", country_code="",
        ...
        self.region_data = {"code": self.region_code, "name": self.region,
                            "country": {
                                "code": self.country_code,
                                "name": self.country_name}}
        self.city_data = {"code": self.city, "name": self.city,
                          "state": self.region_data,
        ...
        self.location_data = {"city": self.city_data,
                              "coordinate": self.coordinate_data,
...
class Database(object):
    def __init__(self, path='sqlite:///mycroft.db', debug=False):
        ...
        self.users = Table('users', self.metadata,
                           Column('mail', String, primary_key=True),
                           Column('name', String),
        ...
        device_association = Table('users_devices', self.metadata,
                                   ...
                                   Column('uuid', Integer,
                                          ForeignKey('users.mail')),
                                   )

        user_association = Table('users_ips', self.metadata,
                                 Column('mail', Integer,
                                        ForeignKey('ips.ip_address')),
        ...
        location_association = Table('configs_location', self.metadata,
                                     Column('config_id', Integer, ForeignKey(
                                         'locations.location_id')),
                                     Column('location_id', Integer,
        ...
        sound_association = Table('sounds_config', self.metadata,
                                  Column('sound_name', Integer,
                                         ForeignKey('configs.config_id')),
                                  Column('config_id', Integer,
                                         ForeignKey('sounds.sound_name')),
                                  )

        hotword_association = Table('hotwords_config', self.metadata,
                                    Column('config_id', Integer,
                                           ForeignKey('hotwords.hotword_id')),
        ...
        tts_association = Table('tts_config', self.metadata,
                                Column('config_id', Integer,
                                       ForeignKey('tts.tts_id')),
                                Column('tts_id', Integer,
                                       ForeignKey('configs.config_id')),
                                )

        stt_association = Table('stt_config', self.metadata,
                                Column('config_id', Integer,
                                       ForeignKey('stt.stt_id')),
                                Column('stt_id', Integer,
                                       ForeignKey('configs.config_id')),
                                )

        blacklisted_skills_association = Table('blacklisted_skills_config',
                                               self.metadata,
                                               Column('config_id', Integer,
                                                      ForeignKey(
                                                          'blacklisted_skills.skill_name')),
                                               Column('skill_name', Integer,
                                                      ForeignKey(
                                                          'configs.config_id')),
                                               )

        priority_skills_association = Table('priority_skills_config',
                                            self.metadata,
                                            Column('config_id', Integer,
                                                   ForeignKey(
                                                       'priority_skills.skill_name')),
                                            Column('skill_name', Integer,
                                                   ForeignKey(
                                                       'configs.config_id')), )

        installed_skills_association = Table('devices_installed_skills',
                                             self.metadata,
                                             Column('uuid', Integer,
                                                    ForeignKey(
        ...
        mapper(PSkill, self.priority_skills)
        mapper(BSkill, self.blacklisted_skills)
        ...
        mapper(Device, self.devices, properties={
            ...
            'location': relation(Location,
                                 secondary=device_location_association,
                                 backref='devices'),
        ...
        mapper(Configuration, self.configs, properties={
            ...
            'stt': relation(STT, secondary=stt_association,
                            backref='configs'),
            'blacklisted_skills': relation(BSkill,
                                           secondary=blacklisted_skills_association,
                                           backref='configs'),
            'priority_skills': relation(PSkill,
                                        secondary=priority_skills_association,
                                        backref='configs'),
    ...
    def get_user_by_device(self, uuid):
        return self.session.query(User).filter_by(uuid=uuid).all()

    def get_user_by_device_name(self, name):
        return self.session.query(User).filter_by(device_name=name).all()

    def get_user_by_ip(self, ip):
        return self.session.query(User).filter_by(ip_adress=ip).all()

    def get_user_by_config(self, config_id):
        return self.session.query(User).filter_by(config_id=config_id).all()

    def get_user_by_wakeword(self, wakeword):
        return self.session.query(User).filter_by(wake_word=wakeword).all()

    def get_user_by_hotword(self, hotword):
        return self.session.query(User).filter_by(hotword_name=hotword).all()

    def get_user_by_lang(self, lang):
        return self.session.query(User).filter_by(lang=lang).all()

    def get_user_by_country(self, country):
        return self.session.query(User).filtert_by(country=country).all()
    ...
    def get_user_by_timezone(self, timezone):
        return self.session.query(User).filter_by(timezone=timezone).all()

    def get_device_by_name(self, name):
        return self.session.query(Device).filter_by(device_name=name).all()

    def get_device_by_user(self, name):
        return self.session.query(Device).filter_by(name=name).all()

    def get_device_by_ip(self, ip):
        return self.session.query(Device).filter_by(ip_adress=ip).all()

    def get_device_by_config(self, config_id):
        return self.session.query(Device).filter_by(config_id=config_id).all()

    def get_device_by_wakeword(self, wakeword):
        return self.session.query(Device).filter_by(wake_word=wakeword).all()

    def get_device_by_hotword(self, hotword):
        return self.session.query(Device).filter_by(
            hotword_name=hotword).all()

    def get_device_by_lang(self, lang):
        return self.session.query(Device).filter_by(lang=lang).all()

    def get_device_by_country(self, country):
        return self.session.query(Device).filter_by(country=country).all()

    def get_device_by_city(self, city):
        return self.session.query(Device).filter_by(city=city).all()

    def get_device_by_timezone(self, timezone):
        return self.session.query(Device).filter_by(timezone=timezone).all()

    def get_config_by_user(self, name):
        return self.session.query(Configuration).filter_by(name=name).all()

    def get_config_by_device(self, uuid):
        return self.session.query(Configuration).filter_by(uuid=uuid).all()

    def get_config_by_device_name(self, name):
        return self.session.query(Configuration).filter_by(
            device_name=name).all()

    def add_user(self, name=None, mail=None, password=None, last_seen=0,
                 api=""):
        user = User(name, mail, password, last_seen, api)
        self.session.add(user)
        self.session.commit()

    def add_device(self, uuid, name="", expires_at=0, accessToken="",
                   refreshToken=""):
        device = Device(uuid, name, expires_at, accessToken, refreshToken)
        self.session.add(device)
        self.session.commit()

    def add_config(self, config_id, lang="en-US", system_unit="metric",
                   time_format="full", date_format="DMY", opt_in=False,
                   confirm_listening=True, skills_dir="/opt/mycroft/skills",
                   play_wav_cmd="paplay %1 --stream-name=mycroft-voice",
                   play_mp3_cmd="mpg123 %1", auto_update=False,
                   ...
                   listener_energy_ratio=1.5, wake_word="hey mycroft",
                   stand_up_word="wake up", wake_word_upload=False):
        config = Configuration(config_id, lang, system_unit,
                               time_format, date_format, opt_in,
                               confirm_listening, skills_dir,
                               play_wav_cmd,
                               play_mp3_cmd, auto_update,
                               listener_sample_rate, listener_channels,
                               record_wake_words, record_utterances,
                               phoneme_duration, listener_multiplier,
                               listener_energy_ratio, wake_word,
                               stand_up_word, wake_word_upload)
